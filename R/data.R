...
#' }
#'
#' @description  This dataset contains the 5738 full rankings resulting from the American Psychological Association (APA)
...
#' @format A list containing:
#' \describe{
#'   \item{data}{A matrix of size 21*8 containing the 21 Premier League seasons. Each row corresponding to one ranking
#' (ranking representation).
#'
#'              The ranking representation r=(r_1,...,r_m) contains the  ranks assigned to the objects, and means that the
#' ith object is in r_ith position.
#'
#'               For example, if the ranking representation of a rank is (4,3,1,2,5), it means that judge ranks the first
#' object in 4th position, second object in 3rd position, ...}
#'   \item{frequency}{matrix of size 21*9. Each row corresponds to one of the 21 different observed rankings, and the
#' last column contains the observation frequency.}
#'   \item{m}{the size of the rankings (m=c(4,4) ).}
#' }
#'
#' @description  This dataset is composed of the rankings (in ranking notation) of the "Big Four" English football teams
...
#'
#' In 2000-2001, Arsenal and Chelsea had the same UEFA coefficient and then are tied. UEFA ranking is (1, 4, 2, 2) for
...
#'
#' \url{https://fr.uefa.com/nationalassociations/uefarankings/club/}
#'
#' @examples
#' data(big4)
#'
...
#' @title Multidimensional partial rank data: eurovision
#' @docType data
#' @aliases eurovision
#' @name eurovision
#' @format A list containing:
#' \describe{
#'   \item{data}{ A matrix of size 34*48. Each row corresponds to the ranking representation of a multidimensional ranking.
#'                Columns 1 to 8 correspond to the 2007 contest, columns 9 to 18 to the 2008 contest, etc...
#'
#'                The ranking representation r=(r_1,...,r_m) contains the  ranks assigned to the objects, and means that
...
#'   }
#'
#'   \item{frequency}{A matrix of size 34*49 containing the different multidimensional rankings. The 48 first columns are
#' the same as in data, and the last column contains the frequency (1 for all ranks).}
#'
...
#'
#' The number of rankings is 33, corresponding to the 33 European countries having participated to those
#' six editions of the contest.
#'
#' All the rankings are partial since none country has ranked this 8 countries in its 10 preferences. Missing ranking
#' elements are zeros.
#'
...
#' @title Multidimensional rank data: quiz
#' @docType data
#' @aliases quiz
#' @name quiz
#' @format A list containing:
#' \describe{
#'   \item{data}{a matrix of size 70*16. The student's answers are in row and the 16 columns correspond to the 4 rankings
#' (for the 4 quizzes) of size 4 (ranking representation).
#'
#'   The ranking representation r=(r_1,...,r_m) contains the  ranks assigned to the objects, and means that the ith object is
#' in r_ith position.
#'
#' For example, if the ranking representation of a rank is (4,3,1,2,5), it means that judge ranks the first object in 4th
#' position, second object in 3rd position, ...}
#' \item{frequency}{a matrix of size 63*17. Each row corresponds to one of the 63 different observed rankings
#' (ranking representation). Each row contains 4 ranks of size 4 and a last column for the frequency.}
#' \item{m}{a vector with the sizes of the ranks for each dimension.}
#'
#' }
#'
#' @description This dataset contains the answers of 70 students (40 of third year and 30 of fourth year)
...
#'       A=Victor Hugo, B=Molière, C=Albert Camus, D=Jean-Jacques Rousseau.}
#'
#'   \item{Football Quiz}{
#'     This quiz consists of ranking four national football teams according to increasing number of wins in the football
#' World Cup: A=France, B=Germany, C=Brazil, D=Italy.}
#'
...
#'
#'   \item{Cinema Quiz}{
#'     This quiz consists of ranking four Tarantino's movies according to chronological order:
...
#' object in 4th position, second object in 3rd position, ...}
#'
#'   \item{frequency}{a matrix with 123 different ranks of size 7. In each row the first 7 columns correspond to one
#' observed ranking and the last column contains the observation frequency.}
...
#' @format A list containing:
#' \describe{
#'   \item{data}{A matrix of size 98*5 containing the 98 answers. Each row corresponding to one ranking
#' (ranking representation).
#'
#'               The ranking representation r=(r_1,...,r_m) contains the  ranks assigned to the objects, and means that
#' the ith object is in r_ith position.
#'
#'               For example, if the ranking representation of a rank is (4,3,1,2,5), it means that judge ranks the
#' first object in 4th position, second object in 3rd position, ...}
#'  \item{frequency}{matrix of size 15*6. Each row corresponds to one of the 15 different observed rankings, and the
#' last column contains the observation frequency.}
